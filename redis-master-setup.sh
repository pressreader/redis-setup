...
echo " 2: ... port $REDIS_PORT"
echo " 3: ... dir /var/lib/$REDIS_INSTANCE_NAME"
echo " 4: ... loglevel notice"
echo " 5: ... logfile /var/log/$REDIS_INSTANCE_NAME/redis.log"
echo " 6: ... #save 900 1"
echo " 7: ... #save 300 10"
echo " 8: ... #save 60 10000"

sudo sed -e "s/^daemonize no$/daemonize yes/" -e "s/^port 6379$/port $REDIS_PORT/" -e "s/^dir \.\//dir \/var\/lib\/$REDIS_INSTANCE_NAME\//" -e "s/^loglevel verbose$/loglevel notice/" -e "s/^logfile stdout$/logfile \/var\/log\/$REDIS_INSTANCE_NAME\/redis.log/" -e "s/^save 900 1$/#save 900 1/" -e "s/^save 300 10$/#save 300 10/" -e "s/^save 60 10000$/#save 60 10000/" redis-$REDIS_VER/redis.conf > redis_tmp.conf
